...
- (void)tableMenuWillShowInCell:(MultiFunctionCell *)cell{
    ...
    self.isCellMenuOn = YES;
    self.activeCell = cell;
    if (self.multiTableDelegate && [self.multiTableDelegate respondsToSelector:@selector(cellWillShowRightMenu)]) {
        [self.multiTableDelegate cellWillShowRightMenu];
    }
}
- (void)tableMenuDidHideInCell:(MultiFunctionCell *)cell{
    ...
    self.activeCell = nil;
    if (self.multiTableDelegate && [self.multiTableDelegate respondsToSelector:@selector(cellDidHideRightMenu)]) {
        [self.multiTableDelegate cellDidHideRightMenu];
    }
}
...
#if DEBUG
    NSLog(@"%s, %d", __FUNCTION__, isCellMenuOn);
#endif
